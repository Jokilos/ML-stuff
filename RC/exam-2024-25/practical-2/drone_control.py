...
if __name__ == '__main__':
    ...
    pid_roll = PID(
        gain_prop = 0.01, gain_int = 0.002, gain_der = 0.003,
        sensor_period = model.opt.timestep, output_limits=(-0.2, 0.2)
    )
    ...
    pid_yaw = PID(
        gain_prop = 0.04, gain_int = 0, gain_der = 0.03,
        sensor_period = model.opt.timestep, output_limits=(-0.2, 0.2)
    )

    pid_altitude = PID(
        gain_prop = 0.2866, gain_int = 0.00, gain_der = 0.5,
        sensor_period = model.opt.timestep, output_limits = (-10, 10) 
    )

    pid_y = PID(
        gain_prop = 5, gain_int = 0.00, gain_der = 7,
        sensor_period = model.opt.timestep, output_limits = (-10, 10) 
    )

    pid_x = PID(
        gain_prop = 0.5, gain_int = 0, gain_der = 0.3,
        sensor_period = model.opt.timestep, output_limits = (-10, 10) 
    )
    # END OF TODO
    ...
    for i in range(SIM_TIME):
        ...
        desx, desy, desired_z = pos_target
        # desired_z = pos_target[2]
        desired_thrust = 3.2496 + pid_altitude.output_signal(desired_z, [current_pos[2], previous_pos[2]])

        desired_roll = -pid_y.output_signal(desy, [current_pos[1], previous_pos[1]]) 
        desired_pitch = pid_x.output_signal(desy, [current_pos[0], previous_pos[0]]) 
