...
def stitch_images(points_left, points_right, imgs = None, filenames = None, leftcentric = True):
    ...
    if leftcentric:
        ...
        span_right, right = apply_homography(H, img = right)
    else:
        H = find_homography(points_left, points_right)
        span_right = np.vstack([np.zeros(2), list(right.shape)[:2][::-1]]).T
        span_left, left = apply_homography(H, img = left)
    ...
    put_on_image(new_right, right, subtract_spans(span_right, span, list(new_size[:2])))
...
def unpack_npz(npz, best_points = 15):
    ...
    k0[:, 1] += SUPERGLUE_MARGIN
    k1[:, 1] += SUPERGLUE_MARGIN 
...
def get_sg_point_pairs(names):
    ...
    npz = np.load(path)
    kleft, kright, matches, _ = unpack_npz(npz)
    right_matches = kright[matches]

    return kleft, right_matches

def stitch2_superglue(imgs = None, filenames = None, script_debug = False, leftcentric = True):
    left, right = inputs_parse(imgs, filenames)
    ...
    for name, file in list(zip(names, [left, right])):
        margin = 100
        h = file.shape[0]
        file = file[np.arange(margin, h - margin)]
...
def stitch5_superglue(imgs = None, filenames = None, script_debug = False):
    imgs = np.array(inputs_parse(imgs, filenames))
    
    first, *rest = imgs[[2, 1, 3, 0, 4]]
    inputs = list(zip(rest, ['L', 'R', 'L', 'R']))
    ...
    for file, side in inputs:
        ...
        current = stitch2_superglue(imgs = argfiles, leftcentric = side == 'R')
...
def task7_snippet():
    ...
    # Turns out to be *much* harder task for the program
    # filenames1 = [f'data3/img{i}.png' for i in range(1,6)][::-1]
